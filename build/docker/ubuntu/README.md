...
docker run -it --rm -h $CODENAME --name $CODENAME -v $OUTPUT:/root/output -e "http_proxy=$http_proxy"  -e "https_proxy=$http_proxy" -e "ftp_proxy=$http_proxy" -e 'PKG_CONFIG_PATH=/usr/local/lib/pkgconfig' -e "OUTPUT=/root/output" ubuntu:$CODENAME
```
